...
import io.chandler.cosc4503.assign1.Lexer.BuildingTypes.*
import io.chandler.cosc4503.assign1.Lexer.State.*
import java.io.File
		...
		for (t in tokens) {
			when (t.type) {
				Token.Type.STRING -> {
					var s = t.data
					when (s) {
						"namespace" -> println("NAMESPACE $s")
						"using" -> println("USING $s")
						"include" -> println("INCLUDE $s")
						else -> {
							if (isDigit(s)) println("NUM $s")
							else if (isValidID(s)) println("ID $s")
						} 
					}
				}
				...
				Token.Type.OPEN -> {
					for (c in t.data) {
						// ' " ( [ {
						when (c) {
							'\'' -> println("L_SINGLE_QUOTE $c")
							'"'  -> println("L_DOUBLE_QUOTE $c")
							'('  -> println("L_PAREN $c")
							'['  -> println("L_SQUARE $c")
							'{'  -> println("L_CURLY $c")
						}
				...
				Token.Type.CLOSE -> {
					for (c in t.data) {
						// ' " ( [ {
						when (c) {
							'\'' -> println("R_SINGLE_QUOTE $c")
							'"'  -> println("R_DOUBLE_QUOTE $c")
							')'  -> println("R_PAREN $c")
							']'  -> println("R_SQUARE $c")
							'}'  -> println("R_CURLY $c")
						}
